...
| **Direction** — write `"ASC"` or `"DESC"` | A convention, not a validated rule: the direction is interpolated into the query as written, so a value outside these two fails as a database error — still a `400` response, but carrying the database's message instead of the `Invalid order column name` validation error. Because it is never validated or escaped, always send a literal direction; never build it from user input. Omitting the direction defaults to `"ASC"`. |
