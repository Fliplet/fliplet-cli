...
- **Parameters:**
  - `nameOrId` (String or Number) — The action name or numeric ID
  - `payload` (Optional Object) — Data to pass as `context.payload`.
- **Returns:** Promise with the environment-specific result described above. Server execution failure rejects the promise; the REST endpoint responds with HTTP 500, status `EXECUTION_FAILED`, and a fixed safe error message. Recognized failures explain the category, such as timeout or blocked request; otherwise the message is `Server action execution failed.` See [execution failures](#execution-failures). An action returning `{ success: false }` is still a completed execution: inspect `result.data.success` for that business outcome.
...
These codes can be handled inside the action. An uncaught error produces a safe execution failure for the caller. Recognized categories retain a fixed message and log code; arbitrary action/provider exception text is not returned by the run endpoint. See [execution failures](#execution-failures).
...
The fields below describe server-only actions (`environment: 'server'`). Their logs omit payloads, returned data and arbitrary exception details because those values can contain credentials. For `any` actions run on the server, payloads and successful results may be retained; failed runner results are omitted and exception messages are replaced with fixed safe messages. Do not put credentials in payloads or return values. Server console output is not forwarded to platform logs.
...
| `error` | Object | Failure only: `{ "code": "EXECUTION_FAILED", "message": "Server action execution failed." }`, or a recognized code and fixed message listed under [execution failures](#execution-failures) |
...
```json
{
  ...
  "data": {
    ...
    "returnedSuccess": null,
    "error": { "code": "EXECUTION_FAILED", "message": "Server action execution failed." }
  }
}
```
...
For server execution, `runWithResult()` rejects when execution fails. The REST response has HTTP 500 and status `EXECUTION_FAILED`; its `error` is a fixed message from the table below. Execution logs store the corresponding `error.code` and `error.message`. Arbitrary exception text and stacks are omitted because they may contain credentials. This applies to both `server` and `any` actions executed on the server.

| Log `error.code` | Fixed message |
|---|---|
| `TIMEOUT` | The action or its provider request timed out. Check the provider and reduce the work per action. |
| `LIMIT` | The action exceeded a data or request limit. Reduce the payload, response size or number of requests. |
| `POLICY` | The action or provider request was blocked by the execution policy. Check the HTTPS origins and request options. |
| `TRANSPORT` | The provider request could not be completed. Check provider availability and connectivity. |
| `UNSUPPORTED` | The provider response format is unsupported. Request a UTF-8 text or JSON response. |
| `ACTION_OWNERSHIP_INVALID` | The action could not be linked to its owning app. Check the app and action configuration. |
| `INTEGRATION_ORIGINS_INVALID` | The provider origins are invalid. Configure at most 20 HTTPS origins on a server-only action. |
| `INTEGRATION_PAYLOAD_TOO_LARGE` | The action invocation or result exceeds 5 MiB. Reduce its size. |
| `LOCAL_ACTION_EXECUTION_UNAVAILABLE` | Server action execution is unavailable in the local environment. |
| `EXECUTION_FAILED` | Server action execution failed. |

A successful `run()` submission means execution started; it does not confirm completion. Check the execution logs for its eventual outcome. Returning `{ success: false, error: 'NOT_CONFIGURED' }` from action code is a completed execution with a business-level failure, not an exception: the caller receives that returned data. Keep such returned errors deliberately safe and specific.
