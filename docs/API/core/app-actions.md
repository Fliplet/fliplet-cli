...
Fliplet.App.Actions.runWithResult('test-action').then(function (result) {
  // If our app action working then it should return the entries from the data source query
  console.log(result)
});
...
Fliplet.Page.onRemoteExecution(function (payload) {
  return Fliplet.DataSources.connect(123)
    ...
    .then(function (records) {
        //In order to see the data currently we need to return it in promise so that we can log in when the action is called above. 
