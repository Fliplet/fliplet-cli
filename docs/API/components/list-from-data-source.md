...
- **options** (Object) A map of options to pass as the value of the persistant variable.
  ...
  - **previousScreen** (Boolean or String) If you want to return to the previous screen when closing a list item, set to true or a string. The string will be executed via `eval()` before the user is returned to the previous screen. We recommend only using this option when you know there will only be one entry as a result. (**Default**: false)
