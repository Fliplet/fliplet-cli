...
Encode text into QR code or barcode and return it as a complete Base64 **data URL** (with the `data:image/png;base64,` prefix already included). Assign the result directly to an `<img>`'s `src` attribute — do **not** prepend the prefix yourself.
...
```js
Fliplet.Barcode.encode(text).then(function (dataUrl) {
  // dataUrl (String) e.g. "data:image/png;base64,iVBORw0KGgoAAA..."
  document.getElementById('qr').src = dataUrl;     // ✅ correct — assign as-is
  // document.getElementById('qr').src = 'data:image/png;base64,' + dataUrl; // ❌ DON'T — double prefix breaks the image
}).catch(function (error) {
  // encoding failed
});
```
...
```js
Fliplet.Barcode.encode(text, options).then(function (dataUrl) {
  // dataUrl (String) Complete data URL — assign directly to <img src>
}).catch(function (error) {
  // encoding failed
});
```
