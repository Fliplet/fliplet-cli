...
**Error handling.** Surface `error.message` to the user; check `error.name` FIRST in the order below, and only fall through to the HTTPS check after no name matched:
...
Then, **as a final fallback on the web path only**: if no `error.name` matched AND `window.location.protocol !== 'https:'` AND `window.location.hostname !== 'localhost'`, return "Browser camera access needs HTTPS." Do NOT run this HTTPS check on the native code path — Cordova WebViews can load from `file://`, `ionic://localhost`, or `https://localhost` depending on platform and plugin version, so the check would misfire and tell native users their HTTPS is wrong. Practically, gate the HTTPS branch behind the same `isWeb` boolean you used to choose the scanner.
